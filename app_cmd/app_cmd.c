...
/* 底盘通信对话：CAN1 / IDSEQ / PROTO_CUSTOM（帧 = [0xA5][seq][payload 1B][CRC8][0x5A]）。
 * 云台侧 tx_id=0x110 段、rx_id=0x100 段（底盘侧对调）；ID 段与 CAN1 现有占用
...
COMM_DEF(chassis_comm, MEDIA_CAN_IDSEQ, CUSTOM, CUSTOM, chassis_gimbal_data_t, 28, gimbal_chassis_data_t, 4, UNPACK_IN_ISR);

static gimbal_chassis_data_t chassis_rx_data = {0}; // 底盘→云台（on_frame 同步拷贝）
static gimbal_chassis_data_t chassis_tx_data = {0}; // 云台→底盘（业务填写后 CommSend）
...
{
    memcpy(&chassis_rx_data, payload, sizeof(chassis_rx_data));
}
...
void AppCmdInit(void)
{
    ...
    CommRegister(&chassis_comm);
    CommConfig(&chassis_comm, &chassis_comm_cfg);
}
...
ITCM_RAM void AppCmdRun(void)
{
    ...
    // 4. 底盘通信：每周期发一帧（接收已由 CAN 中断写入 chassis_rx_data）
    chassis_tx_data.placeholder += 10; /* TODO 填实际数据（如底盘需要的云台 yaw 等） */
    CommSend(&chassis_comm, (uint8_t *)&chassis_tx_data);
}
